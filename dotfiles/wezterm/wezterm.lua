...
config.keys = {
  ...
  { key = ',', mods = 'CMD', action = wezterm.action_callback(function(_, _)
    wezterm.open_with(wezterm.config_file)
  end)},
  { key = 'Return', mods = 'SHIFT', action = action.SendString('\x1b[13;2u') },
...
config.mouse_bindings = {
  ...
  -- { event = { Down = { streak = 1, button = 'Middle' } }, mods = 'NONE', action = wezterm.action.CloseCurrentTab { confirm = false } },
}
